                                ...
                                <div class="flex items-center">
                                    <div class="ml-4">
                                        <div class="text-sm leading-5 font-medium text-gray-900">
                                            <a href="{{ route('builds.show', [$favorite->favoriteable->id]) }}">{{ $favorite->favoriteable->name }}</a>
                                        </div>
                                        <div class="text-sm leading-5 text-gray-500">{{ \Str::limit($favorite->favoriteable->description, 55) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">
                                    <x-character-icon name="{{ $favorite->favoriteable->character->name }}"/>
                                    {{ $favorite->favoriteable->character->name }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">
                                    @foreach($favorite->favoriteable->mods->groupBy('gun_id') as $mods)
                                    <x-gun-icon :name="$mods[0]->gun->name" size="16" />
                                    @endforeach
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">
                                {{ $favorite->favoriteable->updated_at->diffForHumans() }}
                            </td>
...
{{--                                <a href="#" class="text-indigo-600 hover:text-indigo-900">Edit</a>--}}
                                <div class="flex justify-evenly">
                                    ...
                                    <div id="favorite-btn">
                                        {!! Form::model($favorite->favoriteable, ['route' => ['builds.favorites.store', $favorite->favoriteable->id]]) !!}

                                        <button>
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 text-red-600 cursor-pointer" stroke="currentColor" fill="currentColor" viewBox="0 0 20 20"><path d="M10 3.22l-.61-.6a5.5 5.5 0 0 0-7.78 7.77L10 18.78l8.39-8.4a5.5 5.5 0 0 0-7.78-7.77l-.61.61z"></path></svg>
                                        </button>
